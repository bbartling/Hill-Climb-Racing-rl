...
def print_schema(data, indent=0, key_name=None, max_string=100,
                 redact_patterns=(), exclude_patterns=(), visited_ids=None):
    ...
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            ...
            if match_any(k, redact_patterns):
                ...
                continue
            print_schema(v, indent + 1, k, max_string, redact_patterns, exclude_patterns, visited_ids)
        ...
        if obj_example:
            ...
            print_schema(obj_example, indent + 2, None, max_string, redact_patterns, exclude_patterns, visited_ids)
    elif isinstance(data, PRIMITIVES):
        t = type_name(data)
        val = data if isinstance(data, (int, float, bool)) else truncate_str(str(data), max_string)
...
def main():
    ...
    parser.add_argument("json_path", nargs="?", default="hcr_config.json",
                        help="Path to JSON file (default: ./hcr_config.json)")
    parser.add_argument("--redact", nargs="*", default=["ref_img_b64", "/.*_b64$/"],
                        help="Keys or regex to redact (print key but hide value).")
    parser.add_argument("--exclude", nargs="*", default=[],
                        help="Keys or regex to exclude entirely.")
    parser.add_argument("--max-string", type=int, default=100,
