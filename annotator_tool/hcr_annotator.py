...
class MultiScreenAnnotator(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.instructions_label = tk.Label(bottom, text="", font=("Segoe UI", 11), justify=tk.LEFT)
        self.instructions_label.pack(side=tk.LEFT, padx=12)

        btns = tk.Frame(bottom)
        btns.pack(side=tk.RIGHT, padx=10)

        self.load_btn = tk.Button(btns, text="Load Screenshot", command=self._load_screenshot)
    ...
    def _capture_screen_with_monitor_picker(self):
        ...
        if not selector.selected_monitor: return
        time.sleep(0.2)
        with mss() as sct:
    ...
    def _save_current_gameplay_set(self):
        ...
        self.current_img = None
        self.canvas.delete("all")
        messagebox.showinfo("Set Saved", f"Gameplay set saved! You now have {len(self.config_data['gameplay'])} sets.\nLoad another image to continue or click 'Next'.")
        self._update_ui_for_stage()
    ...
    def _try_next(self):
        if self.next_btn["state"] == tk.NORMAL:
            self._next_stage()
        elif self.add_gameplay_btn.winfo_ismapped() and self.add_gameplay_btn["state"] == tk.NORMAL:
            self._save_current_gameplay_set()

    def _save_json(self):
        path = filedialog.asksaveasfilename(defaultextension=".json", initialfile="hcr_config.json", filetypes=[("JSON files", "*.json")])
        if not path: return
    ...
    def _on_press(self, event):
        ...
        self.canvas_rect_id = self.canvas.create_rectangle(self.start_xy, self.start_xy, outline="#FFFFFF", width=2, dash=(5, 3))

    def _on_drag(self, event):
        if self.start_xy: self.canvas.coords(self.canvas_rect_id, *self.start_xy, event.x, event.y)
    ...
    def _canvas_to_image(self, c_coords):
        if not self.current_img or self.scale == 0: return 0, 0
        x = int(max(0, min(self.current_img.size[0], (c_coords[0] - self.offset[0]) / self.scale)))
        y = int(max(0, min(self.current_img.size[1], (c_coords[1] - self.offset[1]) / self.scale)))
        return x, y
