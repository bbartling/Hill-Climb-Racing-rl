...
def process_video(video_in: Path, video_out: Path | None, csv_path: Path | None):
    ...
    if writer_c:
        writer_c.writerow([f"{i / fps:.3f}", i, angle if angle is not None else '',
                           height_px if height_px is not None else '', 0, 0])
    i += 1
...
def record_mode(csv_path: Path, no_display: bool, game_region: dict, fps: float, video_out: Path | None):
        ...
        try:
            while True:
                ...
                if video_out:
                    if writer_v is None:
                        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
                        writer_v = cv2.VideoWriter(str(video_out), fourcc, fps,
...
def preview_region(region: dict):
    ...
    with mss() as sct:
        ...
        frame = cv2.cvtColor(np.array(sct_img), cv2.COLOR_BGRA2BGR)
        cv2.rectangle(frame, (2, 2), (frame.shape[1]-3, frame.shape[0]-3), (0, 255, 255), 2)
        cv2.putText(frame,
...
def select_region_interactive(save_to: Path | None = None):
    ...
    clone = full.copy()
    cv2.putText(clone, "Drag to select region, ENTER to confirm, ESC to cancel",
                (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,0), 3, cv2.LINE_AA)
    cv2.putText(clone, "Drag to select region, ENTER to confirm, ESC to cancel",
    ...
    print(f"Selected region: {region['left']} {region['top']} {region['width']} {region['height']}")

    if save_to:
        save_to.write_text(f"{region['left']} {region['top']} {region['width']} {region['height']}", encoding='utf-8')
...
# -----------------------------
def parse_args():
    p = argparse.ArgumentParser(description='Hill Climb Racing CV tools: images | video | record | region tools')
    ...
    mode.add_argument('--preview-region', action='store_true', help='Live preview of a region (use with --region)')
    mode.add_argument('--select-region', action='store_true', help='Interactively select a region and print/save it')
    ...
    p.add_argument('--record-video-out', type=Path, help='Optional output MP4 for annotated gameplay video (record mode)')

    # Region tools options
    p.add_argument('--save-region-to', type=Path, help='When using --select-region, also save the chosen region to this text file')
...
def main():
    ...
    if args.preview_region:
        if not args.region or len(args.region) != 4:
            raise SystemExit("Please provide --region LEFT TOP WIDTH HEIGHT for --preview-region.")
